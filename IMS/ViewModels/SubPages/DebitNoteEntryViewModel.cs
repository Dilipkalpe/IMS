...
public sealed class DebitNoteEntryViewModel : SubPageViewModelBase
{
    ...
    public DebitNoteEntryViewModel(MainViewModel host) : base(
        host,
        parentTitle: "Debit Note",
        pageTitle: "Add Debit Note",
        pageDescription: "Issue a debit note to a party — amount, GST, and IGST option.",
        iconGlyph: "\uE8C0")
    {
        _host = host;
    ...
    public DebitNoteEntryViewModel(MainViewModel host, int voucherNo) : base(
        host,
        parentTitle: "Debit Note",
        pageTitle: "Edit Debit Note",
        pageDescription: "Update an existing debit note.",
        iconGlyph: "\uE70F")
    {
        _host = host;
        _isEdit = true;
    ...
    public string VoucherTypeLabel => "DEBIT NOTE";
    ...
    private async Task InitializeNewAsync()
    {
        ...
        {
            var next = await ImsApiClient.GetNextDebitNoteNoAsync();
    ...
    private async Task LoadVoucherAsync(int voucherNo)
    {
        ...
        await ApiUiHelper.RunWithApiAsync(async () =>
        {
            var note = await ImsApiClient.GetDebitNoteByNoAsync(voucherNo);
            if (note is null)
                return;
    ...
    private async Task SaveAsync()
    {
        ...
        if (ImsApiClient.IsAvailable)
        {
            ...
            await ApiUiHelper.RunWithApiAsync(async () =>
            {
                var dto = ApiDocumentMapper.FromDebitNoteEntry(this, voucherNo, amount, gstRate, totalAmount);
                if (_isEdit)
                    await ImsApiClient.UpdateDebitNoteByNoAsync(_originalVoucherNo, dto);
                else
                    await ImsApiClient.CreateDebitNoteAsync(dto);
    ...
    internal void SetFromDto(DebitNoteDto dto)
    {
