...
**Cause:** Only TMS is deployed. The `ims.*` domain is attached to the **TMS** Coolify resource (or IMS stack was never created).

---

## Fix in Coolify (do in this order)

### Step A — TMS: keep only `tms.*` domain
