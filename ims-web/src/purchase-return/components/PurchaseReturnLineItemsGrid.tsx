...
import type { PurchaseReturnLineDisplay } from '../lineDisplay';
import type { PurchaseReturnLineItem } from '../types';
import type { usePurchaseReturnDocument } from '../usePurchaseReturnDocument';

type Doc = ReturnType<typeof usePurchaseReturnDocument>;

export interface PurchaseReturnLineItemsGridProps {
  doc: Doc;
  gridRef?: React.RefObject<CorporateDataGridHandle | null>;
  onExitGridEnd?: () => void;
}

function PurchaseReturnLineItemsGridInner({ doc, gridRef, onExitGridEnd }: PurchaseReturnLineItemsGridProps) {
  ...
  const readDisplay = useCallback(
    (lineId: string, key: keyof PurchaseReturnLineDisplay) =>
      displayMapRef.current.get(lineId)?.[key] ?? '',
    [],
  );

  const { deleteLineProtected } = useProtectedSalesLineDelete(PURCHASE_MODULE_CONFIG.purchaseReturn, {
  ...
  const handleDelete = useCallback(
    async (row: PurchaseReturnLineItem, rowIndex: number) => {
  ...
  const columns = useMemo((): DataGridColumn<PurchaseReturnLineItem>[] => {
    const numPatch = (key: keyof PurchaseReturnLineItem) => ({
      setValue: (row: PurchaseReturnLineItem, raw: string) => {
        const n = parseFloat(raw);
        const value = Number.isFinite(n) ? n : 0;
        return { ...row, [key]: value };
      },
      getValue: (row: PurchaseReturnLineItem) => row[key] as number,
    });
    ...
    return [
      ...
      createBalStkColumn<PurchaseReturnLineItem>(),
...
export const PurchaseReturnLineItemsGrid = memo(PurchaseReturnLineItemsGridInner);
