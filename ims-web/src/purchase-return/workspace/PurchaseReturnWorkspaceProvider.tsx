...
import { usePurchaseReturnNavIntent, type PurchaseReturnOpenIntent } from '../context/PurchaseReturnNavIntent';
import { invalidatePurchaseReturnList } from '../repository/listCache';
import { recordToEditor } from '../repository/recordMappers';
import { usePurchaseReturnRepository } from '../repository/PurchaseReturnRepositoryContext';
...
interface PurchaseReturnWorkspaceContextValue {
  tabs: PurchaseReturnTab[];
  activeTabId: string;
  activeTab: PurchaseReturnTab | undefined;
  tabIds: string[];
  ...
  openDocumentInNewTab: (intent: PurchaseReturnOpenIntent) => Promise<void>;
  patchHeader: (tabId: string, key: keyof PurchaseReturnHeader, value: PurchaseReturnHeader[keyof PurchaseReturnHeader]) => void;
  patchLine: (tabId: string, lineId: string, patch: Partial<PurchaseReturnLineItem> | PurchaseReturnLineItem) => void;
...
const PurchaseReturnWorkspaceContext = createContext<PurchaseReturnWorkspaceContextValue | null>(null);

export function PurchaseReturnWorkspaceProvider({
  children,
  lineCount = 0,
}: {
  children: ReactNode;
  ...
  const repository = usePurchaseReturnRepository();
  const { consumeOpenIntent } = usePurchaseReturnNavIntent();
  const initialTabId = 'tab-1';
  const [tabs, setTabs] = useState<PurchaseReturnTab[]>(() => [
    newTabUi(initialTabId, 'New Return', true),
  ]);
  ...
  const loadIntoTab = useCallback(
    async (tabId: string, intent: PurchaseReturnOpenIntent) => {
      ...
      try {
        ...
        if (intent.type === 'edit') {
          ...
          const prefix = normalizeDocPrefix(fresh.header.entryDocPrefix, 'PR');
          ...
          updateDocument(tabId, (d) =>
            applyLoadedDocument(d, d.documentId ?? d.clientDocumentId, header, fresh.lines, 'New return.'),
          );
          return;
        }
  ...
  const openDocumentInNewTab = useCallback(
    async (intent: PurchaseReturnOpenIntent) => {
      const id = `tab-${tabCounter}`;
      setTabCounter((c) => c + 1);
      setTabs((prev) => [
        ...prev.map((t) => ({ ...t, isSelected: false })),
        newTabUi(id, intent.type === 'new' ? 'New Return' : 'Loading…', true),
      ]);
  ...
  const patchHeader = useCallback(
    (tabId: string, key: keyof PurchaseReturnHeader, value: PurchaseReturnHeader[keyof PurchaseReturnHeader]) => {
      updateDocument(tabId, (d) => {
  ...
  const patchLine = useCallback(
    (tabId: string, lineId: string, patch: Partial<PurchaseReturnLineItem> | PurchaseReturnLineItem) => {
      updateDocument(tabId, (d) => {
        const lines = d.lines.map((row) => {
          if (row.id !== lineId) return row;
          return typeof patch === 'object' && 'id' in patch && patch.id === lineId
            ? (patch as PurchaseReturnLineItem)
            : { ...row, ...patch };
        });
  ...
  const commitPrefix = useCallback(
    async (tabId: string) => {
      ...
      const prefix = normalizeDocPrefix(doc.header.entryDocPrefix, 'PR');
      const next = await repository.peekNextNo(prefix);
      updateDocument(tabId, (d) => {
        const header = {
          ...d.header,
  ...
  const duplicateToNewTab = useCallback(
    async (sourceTabId: string) => {
      ...
      const id = `tab-${tabCounter}`;
      setTabCounter((c) => c + 1);
      const prefix = normalizeDocPrefix(source.header.entryDocPrefix, 'PR');
      const next = await repository.peekNextNo(prefix);
      const header = {
        ...source.header,
      ...
      setTabs((prev) => [
        ...prev.map((t) => ({ ...t, isSelected: false })),
        newTabUi(id, 'Duplicated return', true),
      ]);
      setDocuments((prev) => ({
        ...prev,
  ...
  const value = useMemo(
    (): PurchaseReturnWorkspaceContextValue => ({
      tabs,
      activeTabId,
      activeTab,
      tabIds,
  ...
  return (
    <PurchaseReturnWorkspaceContext.Provider value={value}>{children}</PurchaseReturnWorkspaceContext.Provider>
  );
}

export function usePurchaseReturnWorkspace(): PurchaseReturnWorkspaceContextValue {
  const ctx = useContext(PurchaseReturnWorkspaceContext);
  if (!ctx) throw new Error('PurchaseReturnWorkspaceProvider is required.');
  return ctx;
}
