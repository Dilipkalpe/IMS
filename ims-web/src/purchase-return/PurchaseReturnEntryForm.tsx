...
export function PurchaseReturnEntryForm({
  tabId,
  lineCount = 0,
  ...
  const ws = usePurchaseReturnWorkspace();
  const doc = usePurchaseReturnDocument(tabId);
  ...
  const runAction = useCallback(
    async (label: string) => {
      if (label === 'Close' || label === 'Cancel') {
        ...
        navigate(NavKeys.PurchaseReturn);
        return;
      }
  ...
  return (
    <TransactionEntryShell title="Purchase Return">
      ...
      <FormKeyboardScope ref={scopeRef} className="si-entry-layout si-entry-layout--wide-totals" autoFocusFieldKey={autoFocusFieldKey}>
            ...
            <ErpFormGrid>
              <label className="si-field">
                ...
                <input
                  ...
                  readOnly={isEdit}
                  onChange={(e) => doc.updateHeader('entryDocPrefix', normalizeDocPrefix(e.target.value, 'PR'))}
                  onBlur={() => {
        ...
        <PurchaseReturnTotalsRail displayTotals={doc.displayTotals} />
      </FormKeyboardScope>
      </LoadingHost>
