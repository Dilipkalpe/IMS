...
export function usePurchaseReturnDocument(tabId: string) {
  const ws = usePurchaseReturnWorkspace();
  const doc = ws.getDocument(tabId);

  const taxContext = useMemo(() => purchaseReturnGstContext(doc.header), [doc.header]);
  ...
  const updateHeader = useCallback(
    <K extends keyof PurchaseReturnHeader>(key: K, value: PurchaseReturnHeader[K]) => {
  ...
  const updateLine = useCallback(
    (id: string, patch: Partial<PurchaseReturnLineItem> | PurchaseReturnLineItem) => {
