...
export function tabTitleFromState(state: TabDocumentState): string {
  const prefix = state.header.entryDocPrefix || 'DC';
  const no = state.header.billNo?.trim();
  if (no) return `${prefix}-${no}${state.isDirty ? ' *' : ''}`;
  return `New DC${state.isDirty ? ' *' : ''}`;
}

export function newTabUi(id: string, title: string, selected = false): DeliveryChallanTab {
