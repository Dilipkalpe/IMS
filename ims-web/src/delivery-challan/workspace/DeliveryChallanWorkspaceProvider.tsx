...
import { useDeliveryChallanNavIntent, type DeliveryChallanOpenIntent } from '../context/DeliveryChallanNavIntent';
import { invalidateDeliveryChallanList } from '../repository/listCache';
import { recordToEditor } from '../repository/recordMappers';
import { useDeliveryChallanRepository } from '../repository/DeliveryChallanRepositoryContext';
import type { DeliveryChallanHeader, DeliveryChallanLineItem, DeliveryChallanTab } from '../types';
...
interface DeliveryChallanWorkspaceContextValue {
  tabs: DeliveryChallanTab[];
  activeTabId: string;
  activeTab: DeliveryChallanTab | undefined;
  tabIds: string[];
  ...
  openDocumentInNewTab: (intent: DeliveryChallanOpenIntent) => Promise<void>;
  patchHeader: (tabId: string, key: keyof DeliveryChallanHeader, value: DeliveryChallanHeader[keyof DeliveryChallanHeader]) => void;
  patchLine: (tabId: string, lineId: string, patch: Partial<DeliveryChallanLineItem> | DeliveryChallanLineItem) => void;
...
const DeliveryChallanWorkspaceContext = createContext<DeliveryChallanWorkspaceContextValue | null>(null);

export function DeliveryChallanWorkspaceProvider({ children, lineCount = 0 }: { children: ReactNode; lineCount?: number }) {
  const repository = useDeliveryChallanRepository();
  const { validCustomerNames } = useSalesCustomerPicker();
  const { consumeOpenIntent } = useDeliveryChallanNavIntent();
  const initialTabId = 'tab-1';
  const [tabs, setTabs] = useState<DeliveryChallanTab[]>(() => [newTabUi(initialTabId, 'New DC', true)]);
  ...
  const loadIntoTab = useCallback(
    async (tabId: string, intent: DeliveryChallanOpenIntent) => {
      ...
      try {
        ...
        if (intent.type === 'edit') {
          ...
          const prefix = normalizeDocPrefix(fresh.header.entryDocPrefix, 'DC');
          ...
          updateDocument(tabId, (d) =>
            applyLoadedDocument(d, d.documentId ?? d.clientDocumentId, header, fresh.lines, 'New DC.'),
          );
          return;
        }
  ...
  const openDocumentInNewTab = useCallback(
    async (intent: DeliveryChallanOpenIntent) => {
      const id = `tab-${tabCounter}`;
      setTabCounter((c) => c + 1);
      setTabs((prev) => [
        ...prev.map((t) => ({ ...t, isSelected: false })),
        newTabUi(id, intent.type === 'new' ? 'New DC' : 'Loading…', true),
      ]);
    ...
    (id: string): boolean => {
      ...
      if (doc?.isDirty) {
        const ok = confirmDiscardUnsaved(
          ...
          `Tab "${tabs.find((t) => t.id === id)?.title ?? 'DC'}" has unsaved edits.`,
        );
        if (!ok) return false;
      }
      setTabs((prev) => {
  ...
  const patchHeader = useCallback(
    (tabId: string, key: keyof DeliveryChallanHeader, value: DeliveryChallanHeader[keyof DeliveryChallanHeader]) => {
      updateDocument(tabId, (d) => {
  ...
  const patchLine = useCallback(
    (tabId: string, lineId: string, patch: Partial<DeliveryChallanLineItem> | DeliveryChallanLineItem) => {
      updateDocument(tabId, (d) => {
        const lines = d.lines.map((row) => {
          if (row.id !== lineId) return row;
          return typeof patch === 'object' && 'id' in patch && patch.id === lineId
            ? (patch as DeliveryChallanLineItem)
            : { ...row, ...patch };
        });
  ...
  const replaceLinesFromSalesOrders = useCallback(
    (tabId: string, lines: DeliveryChallanLineItem[], soReference: string) => {
      updateDocument(tabId, (d) => {
        const header = { ...d.header, soReference };
        const message = `Loaded ${lines.length} line(s) from sales order(s).`;
  ...
  const prepareNewBill = useCallback(
    async (tabId: string) => {
      ...
      updateDocument(tabId, (d) => resetTabToNewBill(d, header, fresh.lines, 'New DC ready.'));
    },
    [lineCount, repository, updateDocument],
  );
  ...
  const commitPrefix = useCallback(
    async (tabId: string) => {
      ...
      const prefix = normalizeDocPrefix(doc.header.entryDocPrefix, 'DC');
      const next = await repository.peekNextNo(prefix);
      updateDocument(tabId, (d) => {
        const header = {
          ...d.header,
  ...
  const value = useMemo(
    (): DeliveryChallanWorkspaceContextValue => ({
      tabs,
      activeTabId,
      activeTab,
      tabIds,
  ...
  return <DeliveryChallanWorkspaceContext.Provider value={value}>{children}</DeliveryChallanWorkspaceContext.Provider>;
}

export function useDeliveryChallanWorkspace(): DeliveryChallanWorkspaceContextValue {
  const ctx = useContext(DeliveryChallanWorkspaceContext);
  if (!ctx) throw new Error('DeliveryChallanWorkspaceProvider is required.');
  return ctx;
}
