...
function DeliveryChallanWorkspaceInner({ lineCount = 0 }: { lineCount?: number }) {
  ...
  return (
    <RefinedScreenShell className="sales-invoice-workspace-screen">
      <div className="si-workspace" data-wpf-source="Views/DeliveryChallanWorkspaceView.xaml">
...
export function DeliveryChallanWorkspaceScreen({ lineCount = 0 }: { lineCount?: number }) {
  const { isReady } = useDeliveryChallanRepositoryOptional();
  if (!isReady) {
    return (
      <RefinedScreenShell className="sales-invoice-workspace-screen">
        <div className="si-list-toolbar__status">Preparing delivery challan workspace…</div>
      </RefinedScreenShell>
    );
  }
  return (
    <DeliveryChallanWorkspaceProvider lineCount={lineCount}>
      <DeliveryChallanWorkspaceInner lineCount={lineCount} />
    </DeliveryChallanWorkspaceProvider>
  );
}
