...
import { SAMPLE_SALESMEN, SAMPLE_WAREHOUSES } from './mockData';
import { dcTaxHeader } from './taxContext';
import type { FieldError, DeliveryChallanHeader, DeliveryChallanLineItem } from './types';
import { useDeliveryChallanWorkspace } from './workspace/DeliveryChallanWorkspaceProvider';

export function useDeliveryChallanDocument(tabId: string) {
  ...
  const taxContext = useMemo(() => dcTaxHeader(doc.header), [doc.header]);
  ...
  const updateHeader = useCallback(
    <K extends keyof DeliveryChallanHeader>(key: K, value: DeliveryChallanHeader[K]) => {
  ...
  const updateLine = useCallback(
    (id: string, patch: Partial<DeliveryChallanLineItem> | DeliveryChallanLineItem) => {
  ...
  const collectValidationErrors = useCallback((): FieldError[] => {
    ...
    if (!doc.header.billNo?.trim()) next.push({ field: 'billNo', message: 'DC number is required.' });
