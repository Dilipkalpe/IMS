...
import { usePurchaseOrderPrintActions } from '../document/hooks/usePurchaseOrderPrintActions';
...
import { PurchaseOrderLineItemsGrid } from './components/PurchaseOrderLineItemsGrid';
import { PurchaseOrderTotalsRail } from './components/PurchaseOrderTotalsRail';
...
export function PurchaseOrderEntryForm({
  tabId,
  lineCount = 0,
  ...
  const doc = usePurchaseOrderDocument(tabId);
  const { print, savePrintNext } = usePurchaseOrderPrintActions();
  ...
  const runAction = useCallback(
    async (label: string) => {
      if (label === 'Close' || label === 'Cancel') {
        ...
        navigate(NavKeys.PurchaseOrders);
        return;
      }
      ...
      <FormKeyboardScope ref={scopeRef} className="si-entry-layout si-entry-layout--wide-totals" autoFocusFieldKey={autoFocusFieldKey}>
        <div className="si-entry-main">
            ...
            <ErpFormGrid>
              <label className="si-field">
                ...
                <input
                  ...
                  readOnly={isEdit}
                  onChange={(e) => doc.updateHeader('entryDocPrefix', normalizeDocPrefix(e.target.value, 'PO'))}
                  onBlur={() => {
              ...
              <label className="si-field">
                <span className="wpf-subpage-form-label">Order No</span>
                <input
              ...
              <label className="si-field">
                <span className="wpf-subpage-form-label">PO Date</span>
                <input
                  type="date"
          ...
          <section className="si-section si-section--grow si-section--lines-panel">
            <PurchaseOrderLineItemsGrid doc={doc} gridRef={gridRef} onExitGridEnd={focusNarration} />
          </section>
        ...
        <PurchaseOrderTotalsRail displayTotals={doc.displayTotals} />
      </FormKeyboardScope>
      </LoadingHost>
