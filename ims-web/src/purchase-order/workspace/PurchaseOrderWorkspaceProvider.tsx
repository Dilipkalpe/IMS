...
import { usePurchaseOrderNavIntent, type PurchaseOrderOpenIntent } from '../context/PurchaseOrderNavIntent';
import { invalidatePurchaseOrderList } from '../repository/listCache';
import { recordToEditor } from '../repository/recordMappers';
import { usePurchaseOrderRepository } from '../repository/PurchaseOrderRepositoryContext';
import type { PurchaseOrderHeader, PurchaseOrderLineItem, PurchaseOrderTab } from '../types';
...
interface PurchaseOrderWorkspaceContextValue {
  tabs: PurchaseOrderTab[];
  activeTabId: string;
  activeTab: PurchaseOrderTab | undefined;
  tabIds: string[];
  ...
  openDocumentInNewTab: (intent: PurchaseOrderOpenIntent) => Promise<void>;
  patchHeader: (tabId: string, key: keyof PurchaseOrderHeader, value: PurchaseOrderHeader[keyof PurchaseOrderHeader]) => void;
  patchLine: (tabId: string, lineId: string, patch: Partial<PurchaseOrderLineItem> | PurchaseOrderLineItem) => void;
...
const PurchaseOrderWorkspaceContext = createContext<PurchaseOrderWorkspaceContextValue | null>(null);

export function PurchaseOrderWorkspaceProvider({
  children,
  lineCount = 0,
}: {
  children: ReactNode;
  ...
  const repository = usePurchaseOrderRepository();
  const { consumeOpenIntent } = usePurchaseOrderNavIntent();
  const initialTabId = 'tab-1';
  const [tabs, setTabs] = useState<PurchaseOrderTab[]>(() => [
    newTabUi(initialTabId, 'New Order', true),
  ]);
  ...
  const loadIntoTab = useCallback(
    async (tabId: string, intent: PurchaseOrderOpenIntent) => {
      ...
      try {
        ...
        if (intent.type === 'edit') {
          ...
          const prefix = normalizeDocPrefix(fresh.header.entryDocPrefix, 'PO');
          ...
          updateDocument(tabId, (d) =>
            applyLoadedDocument(d, d.documentId ?? d.clientDocumentId, header, fresh.lines, 'New order.'),
          );
          return;
        }
  ...
  const openDocumentInNewTab = useCallback(
    async (intent: PurchaseOrderOpenIntent) => {
      const id = `tab-${tabCounter}`;
      setTabCounter((c) => c + 1);
      setTabs((prev) => [
        ...prev.map((t) => ({ ...t, isSelected: false })),
        newTabUi(id, intent.type === 'new' ? 'New Order' : 'Loading…', true),
      ]);
  ...
  const patchHeader = useCallback(
    (tabId: string, key: keyof PurchaseOrderHeader, value: PurchaseOrderHeader[keyof PurchaseOrderHeader]) => {
      updateDocument(tabId, (d) => {
  ...
  const patchLine = useCallback(
    (tabId: string, lineId: string, patch: Partial<PurchaseOrderLineItem> | PurchaseOrderLineItem) => {
      updateDocument(tabId, (d) => {
        const lines = d.lines.map((row) => {
          if (row.id !== lineId) return row;
          return typeof patch === 'object' && 'id' in patch && patch.id === lineId
            ? (patch as PurchaseOrderLineItem)
            : { ...row, ...patch };
        });
  ...
  const commitPrefix = useCallback(
    async (tabId: string) => {
      ...
      const prefix = normalizeDocPrefix(doc.header.entryDocPrefix, 'PO');
      const next = await repository.peekNextNo(prefix);
      updateDocument(tabId, (d) => {
        const header = {
          ...d.header,
  ...
  const value = useMemo(
    (): PurchaseOrderWorkspaceContextValue => ({
      tabs,
      activeTabId,
      activeTab,
      tabIds,
  ...
  return (
    <PurchaseOrderWorkspaceContext.Provider value={value}>{children}</PurchaseOrderWorkspaceContext.Provider>
  );
}

export function usePurchaseOrderWorkspace(): PurchaseOrderWorkspaceContextValue {
  const ctx = useContext(PurchaseOrderWorkspaceContext);
  if (!ctx) throw new Error('PurchaseOrderWorkspaceProvider is required.');
  return ctx;
}
