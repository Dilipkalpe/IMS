...
import type { QuotationHeader, QuotationLineItem, QuotationTotals } from '../../quotation/types';
import type { PrintableDocumentV1, PrintableLineV1 } from '../contracts/printableDocument';

export interface QuotationUiSnapshot {
...
function formatDocNo(prefix: string, billNo: string): string {
  const p = prefix?.trim() || 'QT';
  const n = billNo?.trim();
  return n ? `${p}-${n}` : p;
}

function mapLine(line: QuotationLineItem, header: QuotationHeader): PrintableLineV1 {
...
export function mapQuotationToPrintableDocument(snapshot: QuotationUiSnapshot): PrintableDocumentV1 {
  ...
  return {
    ...
    header: {
      ...
      documentDate: h.quoteDate,
      dueDate: h.validUntil,
