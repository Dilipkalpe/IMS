...
import type { SalesOrderHeader, SalesOrderLineItem, SalesOrderTotals } from '../../sales-order/types';
import type { PrintableDocumentV1, PrintableLineV1 } from '../contracts/printableDocument';

export interface SalesOrderUiSnapshot {
...
function formatDocNo(prefix: string, billNo: string): string {
  const p = prefix?.trim() || 'SO';
  const n = billNo?.trim();
  return n ? `${p}-${n}` : p;
}

function mapLine(line: SalesOrderLineItem, header: SalesOrderHeader): PrintableLineV1 {
...
export function mapSalesOrderToPrintableDocument(snapshot: SalesOrderUiSnapshot): PrintableDocumentV1 {
