...
import type { FieldError, SalesOrderHeader, SalesOrderLineItem } from './types';
import { useSalesOrderWorkspace } from './workspace/SalesOrderWorkspaceProvider';

export function useSalesOrderDocument(tabId: string) {
  ...
  const taxContext = useMemo(
    () =>
      taxContextFromHeader({
        placeOfSupply: doc.header.placeOfSupply,
        sellerGstin: doc.header.sellerGstin,
        customerGstin: doc.header.customerGstin,
      }),
  ...
  const updateHeader = useCallback(
    <K extends keyof SalesOrderHeader>(key: K, value: SalesOrderHeader[K]) => {
  ...
  const updateLine = useCallback(
    (id: string, patch: Partial<SalesOrderLineItem> | SalesOrderLineItem) => {
  ...
  const collectValidationErrors = useCallback((): FieldError[] => {
    ...
    const touchedLines = doc.lines.filter(
      (l) => l.qty > 0 || l.productRetailCode.trim() || l.itemDescription.trim(),
    );
    if (!touchedLines.some((l) => l.qty > 0)) {
      next.push({
        field: 'lines',
        message: 'Add at least one line item.',
      });
    }
    touchedLines.forEach((l, i) => {
      if (l.qty <= 0) {
        next.push({ field: `qty-${l.id}`, message: `Line ${i + 1}: Qty must be greater than zero.` });
      }
    });
  ...
  const afterSaveNavigation = useCallback(
    async (label: string) => {
      if (label === 'Save') {
        return ws.closeTabWithoutConfirm(tabId);
      }
