...
import { useSalesOrderNavIntent, type SalesOrderOpenIntent } from '../context/SalesOrderNavIntent';
...
import { useSalesOrderRepository } from '../repository/SalesOrderRepositoryContext';
...
interface SalesOrderWorkspaceContextValue {
  tabs: SalesOrderTab[];
  activeTabId: string;
  activeTab: SalesOrderTab | undefined;
  tabIds: string[];
  ...
  openDocumentInNewTab: (intent: SalesOrderOpenIntent) => Promise<void>;
  patchHeader: (tabId: string, key: keyof SalesOrderHeader, value: SalesOrderHeader[keyof SalesOrderHeader]) => void;
  patchLine: (tabId: string, lineId: string, patch: Partial<SalesOrderLineItem> | SalesOrderLineItem) => void;
...
const SalesOrderWorkspaceContext = createContext<SalesOrderWorkspaceContextValue | null>(null);

export function SalesOrderWorkspaceProvider({
  children,
  lineCount = 0,
}: {
  children: ReactNode;
  ...
  const repository = useSalesOrderRepository();
  const { validCustomerNames } = useSalesCustomerPicker();
  const { consumeOpenIntent } = useSalesOrderNavIntent();
  const initialTabId = 'tab-1';
  const [tabs, setTabs] = useState<SalesOrderTab[]>(() => [
  ...
  const loadIntoTab = useCallback(
    async (tabId: string, intent: SalesOrderOpenIntent) => {
      ...
      try {
        ...
        if (intent.type === 'edit') {
          ...
          const prefix = normalizeSoPrefix(fresh.header.entryDocPrefix);
          ...
          updateDocument(tabId, (d) =>
            applyLoadedDocument(
              ...
              header,
              fresh.lines,
              'New order — fill customer and line items, then Save.',
            ),
          );
          return;
        }
  ...
  const openDocumentInNewTab = useCallback(
    async (intent: SalesOrderOpenIntent) => {
  ...
  const patchHeader = useCallback(
    (tabId: string, key: keyof SalesOrderHeader, value: SalesOrderHeader[keyof SalesOrderHeader]) => {
      updateDocument(tabId, (d) => {
  ...
  const prepareNewBill = useCallback(
    async (tabId: string) => {
      ...
      const next = await repository.peekNextNo(prefix);
      const salesMan = getAuthSession()?.user?.fullName ?? '';
      const header = { ...fresh.header, entryDocPrefix: next.soPrefix, billNo: String(next.docNo), salesMan };
  ...
  const commitPrefix = useCallback(
    async (tabId: string) => {
      ...
      const prefix = normalizeSoPrefix(doc.header.entryDocPrefix);
      const next = await repository.peekNextNo(prefix);
      updateDocument(tabId, (d) => {
        const header = { ...d.header, entryDocPrefix: next.soPrefix, billNo: String(next.docNo) };
  ...
  const duplicateToNewTab = useCallback(
    async (sourceTabId: string) => {
      ...
      setTabs((prev) => [
        ...prev.map((t) => ({ ...t, isSelected: false })),
        newTabUi(id, 'Duplicated order', true),
      ]);
      setDocuments((prev) => ({
        ...prev,
  ...
  const value = useMemo(
    (): SalesOrderWorkspaceContextValue => ({
      tabs,
      activeTabId,
      activeTab,
      tabIds,
  ...
  return (
    <SalesOrderWorkspaceContext.Provider value={value}>{children}</SalesOrderWorkspaceContext.Provider>
  );
}

export function useSalesOrderWorkspace(): SalesOrderWorkspaceContextValue {
  const ctx = useContext(SalesOrderWorkspaceContext);
  if (!ctx) throw new Error('SalesOrderWorkspaceProvider is required.');
  return ctx;
}
