...
function QuotationWorkspaceInner({ lineCount = 0 }: { lineCount?: number }) {
  ...
  return (
    <RefinedScreenShell className="sales-invoice-workspace-screen">
      <div className="si-workspace" data-wpf-source="Views/QuotationWorkspaceView.xaml">
...
export function QuotationWorkspaceScreen({ lineCount = 0 }: { lineCount?: number }) {
  const { isReady } = useQuotationRepositoryOptional();
  if (!isReady) {
    return (
      <RefinedScreenShell className="sales-invoice-workspace-screen">
        <div className="si-list-toolbar__status">Preparing quotation workspace…</div>
      </RefinedScreenShell>
    );
  }
  return (
    <QuotationWorkspaceProvider lineCount={lineCount}>
      <QuotationWorkspaceInner lineCount={lineCount} />
    </QuotationWorkspaceProvider>
  );
}
