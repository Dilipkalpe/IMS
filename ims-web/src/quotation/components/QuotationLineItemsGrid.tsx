...
import type { QuotationLineDisplay } from '../lineDisplay';
import type { QuotationLineItem } from '../types';
import type { useQuotationDocument } from '../useQuotationDocument';

type Doc = ReturnType<typeof useQuotationDocument>;

export interface QuotationLineItemsGridProps {
  doc: Doc;
  gridRef?: React.RefObject<CorporateDataGridHandle | null>;
  onExitGridEnd?: () => void;
}

function QuotationLineItemsGridInner({ doc, gridRef, onExitGridEnd }: QuotationLineItemsGridProps) {
  ...
  const readDisplay = useCallback(
    (lineId: string, key: keyof QuotationLineDisplay) =>
      displayMapRef.current.get(lineId)?.[key] ?? '',
    [],
  );

  const { deleteLineProtected } = useProtectedSalesLineDelete(SALES_MODULE_CONFIG.quotation, {
  ...
  const handleDelete = useCallback(
    async (row: QuotationLineItem, rowIndex: number) => {
  ...
  const columns = useMemo((): DataGridColumn<QuotationLineItem>[] => {
    const numPatch = (key: keyof QuotationLineItem) => ({
      setValue: (row: QuotationLineItem, raw: string) => {
        const n = parseFloat(raw);
        const value = Number.isFinite(n) ? n : 0;
        return { ...row, [key]: value };
      },
      getValue: (row: QuotationLineItem) => row[key] as number,
    });
...
export const QuotationLineItemsGrid = memo(QuotationLineItemsGridInner);
