...
import type { FieldError, QuotationHeader, QuotationLineItem } from './types';
import { useQuotationWorkspace } from './workspace/QuotationWorkspaceProvider';

export function useQuotationDocument(tabId: string) {
  ...
  const updateHeader = useCallback(
    <K extends keyof QuotationHeader>(key: K, value: QuotationHeader[K]) => {
  ...
  const updateLine = useCallback(
    (id: string, patch: Partial<QuotationLineItem> | QuotationLineItem) => {
