...
function SalesInvoiceWorkspaceInner({ stressLines }: { stressLines?: number }) {
      ...
      <div className="si-workspace" data-wpf-source="Views/SalesInvoiceWorkspaceView.xaml">
        <div className="si-workspace__tabbar">
          ...
          >
            ...
            <button
              type="button"
              className="wpf-action-button si-workspace__new-tab"
              onClick={() => addTab()}
        ...
        <div className="si-workspace__content" role="tabpanel">
          {mountedTabs.map((tab) => (
            <div
              key={tab.id}
              className="si-workspace__panel"
              hidden={!tab.isSelected}
              aria-hidden={!tab.isSelected}
            >
              <SalesInvoiceEntryForm
                tabId={tab.id}
                lineCount={lineCount}
                autoFocusFieldKey={
                  tab.isSelected ? `entryDocPrefix-${tab.id}-${focusSeed}` : undefined
                }
              />
            </div>
          ))}
...
export function SalesInvoiceWorkspaceScreen({ stressLines: stressLinesProp }: { stressLines?: number }) {
  const { isReady } = useSalesInvoiceRepositoryOptional();
  const stressLines = stressLinesProp ?? readStressLineCountFromLocation();
  if (!isReady) {
    return (
      <RefinedScreenShell className="sales-invoice-workspace-screen">
        <div className="si-list-toolbar__status">Preparing sales invoice workspace…</div>
      </RefinedScreenShell>
    );
  }
  return (
    <SalesInvoiceWorkspaceProvider lineCount={stressLines ?? 0}>
      <SalesInvoiceWorkspaceInner stressLines={stressLines} />
    </SalesInvoiceWorkspaceProvider>
  );
}
