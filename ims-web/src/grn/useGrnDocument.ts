...
export function useGrnDocument(tabId: string) {
  const ws = useGrnWorkspace();
  const doc = ws.getDocument(tabId);

  const taxContext = useMemo(() => grnTaxHeader(doc.header), [doc.header]);
  ...
  const updateHeader = useCallback(
    <K extends keyof GrnHeader>(key: K, value: GrnHeader[K]) => {
  ...
  const updateLine = useCallback(
    (id: string, patch: Partial<GrnLineItem> | GrnLineItem) => {
  ...
  const collectValidationErrors = useCallback((): FieldError[] => {
    ...
    if (!doc.header.billNo?.trim()) next.push({ field: 'billNo', message: 'GRN number is required.' });
