...
import { useGrnRepositoryOptional } from './repository/GrnRepositoryContext';
...
import { GrnWorkspaceProvider, useGrnWorkspace } from './workspace/GrnWorkspaceProvider';

function GrnWorkspaceInner({ lineCount = 0 }: { lineCount?: number }) {
  ...
  return (
    <RefinedScreenShell className="sales-invoice-workspace-screen">
      <div className="si-workspace" data-wpf-source="Views/GrnWorkspaceView.xaml">
...
export function GrnWorkspaceScreen({ lineCount = 0 }: { lineCount?: number }) {
  const { isReady } = useGrnRepositoryOptional();
  if (!isReady) {
    return (
      <RefinedScreenShell className="sales-invoice-workspace-screen">
        <div className="si-list-toolbar__status">Preparing GRN workspace…</div>
      </RefinedScreenShell>
    );
  }
  return (
    <GrnWorkspaceProvider lineCount={lineCount}>
      <GrnWorkspaceInner lineCount={lineCount} />
    </GrnWorkspaceProvider>
  );
}
