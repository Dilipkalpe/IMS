...
import { useSalesReturnNavIntent, type SalesReturnOpenIntent } from '../context/SalesReturnNavIntent';
import { invalidateSalesReturnList } from '../repository/listCache';
import { recordToEditor } from '../repository/recordMappers';
import { useSalesReturnRepository } from '../repository/SalesReturnRepositoryContext';
import type { SalesReturnHeader, SalesReturnLineItem, SalesReturnTab } from '../types';
...
interface SalesReturnWorkspaceContextValue {
  tabs: SalesReturnTab[];
  activeTabId: string;
  activeTab: SalesReturnTab | undefined;
  tabIds: string[];
  ...
  openDocumentInNewTab: (intent: SalesReturnOpenIntent) => Promise<void>;
  patchHeader: (tabId: string, key: keyof SalesReturnHeader, value: SalesReturnHeader[keyof SalesReturnHeader]) => void;
  patchLine: (tabId: string, lineId: string, patch: Partial<SalesReturnLineItem> | SalesReturnLineItem) => void;
...
const SalesReturnWorkspaceContext = createContext<SalesReturnWorkspaceContextValue | null>(null);

export function SalesReturnWorkspaceProvider({
  children,
  lineCount = 0,
}: {
  children: ReactNode;
  ...
  const repository = useSalesReturnRepository();
  const { validCustomerNames } = useSalesCustomerPicker();
  const { consumeOpenIntent } = useSalesReturnNavIntent();
  const initialTabId = 'tab-1';
  const [tabs, setTabs] = useState<SalesReturnTab[]>(() => [
  ...
  const loadIntoTab = useCallback(
    async (tabId: string, intent: SalesReturnOpenIntent) => {
      ...
      try {
        ...
        if (intent.type === 'edit') {
          ...
          const prefix = normalizeDocPrefix(fresh.header.entryDocPrefix, 'SR');
  ...
  const openDocumentInNewTab = useCallback(
    async (intent: SalesReturnOpenIntent) => {
  ...
  const patchHeader = useCallback(
    (tabId: string, key: keyof SalesReturnHeader, value: SalesReturnHeader[keyof SalesReturnHeader]) => {
      updateDocument(tabId, (d) => {
  ...
  const patchLine = useCallback(
    (tabId: string, lineId: string, patch: Partial<SalesReturnLineItem> | SalesReturnLineItem) => {
      updateDocument(tabId, (d) => {
        const lines = d.lines.map((row) => {
          if (row.id !== lineId) return row;
          return typeof patch === 'object' && 'id' in patch && patch.id === lineId
            ? (patch as SalesReturnLineItem)
            : { ...row, ...patch };
        });
  ...
  const commitPrefix = useCallback(
    async (tabId: string) => {
      ...
      const prefix = normalizeDocPrefix(doc.header.entryDocPrefix, 'SR');
      const next = await repository.peekNextNo(prefix);
      updateDocument(tabId, (d) => {
        const header = {
          ...d.header,
  ...
  const value = useMemo(
    (): SalesReturnWorkspaceContextValue => ({
      tabs,
      activeTabId,
      activeTab,
      tabIds,
  ...
  return (
    <SalesReturnWorkspaceContext.Provider value={value}>{children}</SalesReturnWorkspaceContext.Provider>
  );
}

export function useSalesReturnWorkspace(): SalesReturnWorkspaceContextValue {
  const ctx = useContext(SalesReturnWorkspaceContext);
  if (!ctx) throw new Error('SalesReturnWorkspaceProvider is required.');
  return ctx;
}
