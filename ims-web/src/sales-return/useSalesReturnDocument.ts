...
import type { FieldError, SalesReturnHeader, SalesReturnLineItem } from './types';
import { useSalesReturnWorkspace } from './workspace/SalesReturnWorkspaceProvider';

export function useSalesReturnDocument(tabId: string) {
  ...
  const updateHeader = useCallback(
    <K extends keyof SalesReturnHeader>(key: K, value: SalesReturnHeader[K]) => {
  ...
  const updateLine = useCallback(
    (id: string, patch: Partial<SalesReturnLineItem> | SalesReturnLineItem) => {
  ...
  const collectValidationErrors = useCallback((): FieldError[] => {
    ...
    if (!doc.header.billNo?.trim()) next.push({ field: 'billNo', message: 'Return number is required.' });
