...
import { useSalesReturnRepositoryOptional } from './repository/SalesReturnRepositoryContext';
...
import { SalesReturnEntryForm } from './SalesReturnEntryForm';
import { SalesReturnWorkspaceProvider, useSalesReturnWorkspace } from './workspace/SalesReturnWorkspaceProvider';

function SalesReturnWorkspaceInner({ lineCount = 0 }: { lineCount?: number }) {
  ...
  return (
    <RefinedScreenShell className="sales-invoice-workspace-screen">
      <div className="si-workspace" data-wpf-source="Views/SalesReturnWorkspaceView.xaml">
        <div className="si-workspace__tabbar">
          <div
            className="si-workspace__tabs"
            role="tablist"
            aria-label="Sales return tabs"
...
export function SalesReturnWorkspaceScreen({ lineCount = 0 }: { lineCount?: number }) {
  const { isReady } = useSalesReturnRepositoryOptional();
  if (!isReady) {
    return (
      <RefinedScreenShell className="sales-invoice-workspace-screen">
        <div className="si-list-toolbar__status">Preparing sales return workspace…</div>
      </RefinedScreenShell>
    );
  }
  return (
    <SalesReturnWorkspaceProvider lineCount={lineCount}>
      <SalesReturnWorkspaceInner lineCount={lineCount} />
    </SalesReturnWorkspaceProvider>
  );
}
