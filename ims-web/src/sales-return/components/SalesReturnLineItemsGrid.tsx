...
import type { SalesReturnLineDisplay } from '../lineDisplay';
import type { SalesReturnLineItem } from '../types';
import type { useSalesReturnDocument } from '../useSalesReturnDocument';

type Doc = ReturnType<typeof useSalesReturnDocument>;

export interface SalesReturnLineItemsGridProps {
  doc: Doc;
  gridRef?: React.RefObject<CorporateDataGridHandle | null>;
  onExitGridEnd?: () => void;
}

function SalesReturnLineItemsGridInner({ doc, gridRef, onExitGridEnd }: SalesReturnLineItemsGridProps) {
  ...
  const readDisplay = useCallback(
    (lineId: string, key: keyof SalesReturnLineDisplay) =>
      displayMapRef.current.get(lineId)?.[key] ?? '',
    [],
  );

  const { deleteLineProtected } = useProtectedSalesLineDelete(SALES_MODULE_CONFIG.salesReturn, {
  ...
  const handleDelete = useCallback(
    async (row: SalesReturnLineItem, rowIndex: number) => {
  ...
  const columns = useMemo((): DataGridColumn<SalesReturnLineItem>[] => {
    const numPatch = (key: keyof SalesReturnLineItem) => ({
      setValue: (row: SalesReturnLineItem, raw: string) => {
        const n = parseFloat(raw);
        const value = Number.isFinite(n) ? n : 0;
        return { ...row, [key]: value };
      },
      getValue: (row: SalesReturnLineItem) => row[key] as number,
    });
...
export const SalesReturnLineItemsGrid = memo(SalesReturnLineItemsGridInner);
