import { apiFetch } from '../../api/client';
import { appendTransactionListQueryParams } from '../../components/transaction/transactionListQuery';
import { buildSavePayload, recordToEditor } from './recordMappers';
import type {
  SalesReturnListQuery,
  SalesReturnListResult,
  SalesReturnListStats,
  SalesReturnNextNo,
  SalesReturnRecord,
  SalesReturnRepository,
  SaveSalesReturnInput,
  SaveSalesReturnResult,
} from './types';

const BASE = '/api/sales-returns';

export class HttpSalesReturnRepository implements SalesReturnRepository {
  readonly mode = 'http' as const;

  async fetchList(query: SalesReturnListQuery = {}): Promise<SalesReturnListResult> {
    const params = new URLSearchParams();
    appendTransactionListQueryParams(params, query);
    const q = params.toString();
    return apiFetch<SalesReturnListResult>(`${BASE}${q ? `?${q}` : ''}`);
  }

  async fetchStats(): Promise<SalesReturnListStats> {
    const stats = await apiFetch<{
      total: number;
      draft: number;
      open: number;
      confirmed: number;
    }>(`${BASE}/stats`);
    return { total: stats.total, draft: stats.draft, open: stats.open, confirmed: stats.confirmed };
  }

  async loadById(id: string): Promise<SalesReturnRecord> {
    return normalizeRecord(await apiFetch<SalesReturnRecord & { id?: string }>(`${BASE}/${id}`));
  }

  async loadByFormatted(formatted: string): Promise<SalesReturnRecord> {
    const enc = encodeURIComponent(formatted);
    return normalizeRecord(
      await apiFetch<SalesReturnRecord & { id?: string }>(`${BASE}/by-formatted/${enc}`),
    );
  }

  async peekNextNo(prefix?: string): Promise<SalesReturnNextNo> {
    const q = prefix ? `?prefix=${encodeURIComponent(prefix)}` : '';
    return apiFetch<SalesReturnNextNo>(`${BASE}/next-no${q}`);
  }

  async save(input: SaveSalesReturnInput): Promise<SaveSalesReturnResult> {
    const payload = buildSavePayload(input);
    if (input.id) {
      const item = await apiFetch<SalesReturnRecord & { id?: string }>(`${BASE}/${input.id}`, {
        method: 'PUT',
        body: JSON.stringify(payload),
      });
      return { record: normalizeRecord(item), created: false };
    }
    const item = await apiFetch<SalesReturnRecord & { id?: string }>(BASE, {
      method: 'POST',
      body: JSON.stringify(payload),
    });
    return { record: normalizeRecord(item), created: true };
  }

  async deleteById(id: string): Promise<void> {
    await apiFetch(`${BASE}/${id}`, { method: 'DELETE' });
  }
}

function normalizeRecord(raw: SalesReturnRecord & { id?: string }): SalesReturnRecord {
  return { ...raw, _id: String(raw._id ?? raw.id ?? '') };
}

export { recordToEditor };
