...
import { useSalesReturnPrintActions } from '../document/hooks/useSalesReturnPrintActions';
import {
...
import { SalesReturnLineItemsGrid } from './components/SalesReturnLineItemsGrid';
import { SalesReturnTotalsRail } from './components/SalesReturnTotalsRail';
import { useSalesReturnDocument } from './useSalesReturnDocument';
import { useSalesReturnWorkspace } from './workspace/SalesReturnWorkspaceProvider';
import '../sales-invoice/sales-invoice.scss';

export function SalesReturnEntryForm({
  tabId,
  lineCount = 0,
  ...
  const ws = useSalesReturnWorkspace();
  const doc = useSalesReturnDocument(tabId);
  const { print, savePrintNext } = useSalesReturnPrintActions();
  ...
  const runPrintFlow = useCallback(
    async (label: 'Print' | 'Save, Print, Next (F12)') => {
      ...
      if (label === 'Print') {
        ...
        if (outcome.ok) registerPrintPreviousSnapshot('sales-return', snapshot);
  ...
  return (
    <TransactionEntryShell title="Sales Return">
      ...
      <FormKeyboardScope ref={scopeRef} className="si-entry-layout si-entry-layout--wide-totals" autoFocusFieldKey={autoFocusFieldKey}>
        <div className="si-entry-main">
          ...
          <ErpFormSection>
            <ErpFormGrid>
              <label className="si-field">
                ...
                <input
                  ...
                  readOnly={isEdit}
                  onChange={(e) => doc.updateHeader('entryDocPrefix', normalizeDocPrefix(e.target.value, 'SR'))}
                  onBlur={() => {
              ...
              <label className="si-field">
                <span className="wpf-subpage-form-label">Return No</span>
                <input
              ...
              <label className="si-field">
                ...
                <input
                  ...
                  {...{ [FIELD_FOCUS_KEY]: 'returnDate' }}
                  value={h.returnDate}
                  onChange={(e) => doc.updateHeader('returnDate', e.target.value)}
                />
              </label>
              <label className="si-field">
                <span className="wpf-subpage-form-label">Invoice Reference</span>
                <input
                  className="wpf-subpage-form-input"
                  {...{ [FIELD_FOCUS_KEY]: 'invoiceReference' }}
                  value={h.invoiceReference}
                  onChange={(e) => doc.updateHeader('invoiceReference', e.target.value)}
                />
              </label>
              <label className="si-field">
                <span className="wpf-subpage-form-label">Return Reason</span>
                <input
                  className="wpf-subpage-form-input"
                  {...{ [FIELD_FOCUS_KEY]: 'returnReason' }}
                  value={h.returnReason}
            ...
            <ErpFormGrid variant="gst">
              ...
              <label className="si-field">
                <span className="wpf-subpage-form-label">Return Warehouse</span>
                <select
                  className="wpf-subpage-form-combo"
                  {...{ [FIELD_FOCUS_KEY]: 'returnWarehouse' }}
                  value={h.returnWarehouse}
                  onChange={(e) => doc.updateHeader('returnWarehouse', e.target.value)}
                >
                  {RETURN_WAREHOUSES.map((p) => (
                    <option key={p} value={p}>
                      {p}
                    </option>
                  ))}
              ...
              <label className="si-field">
                <span className="wpf-subpage-form-label">QC Remark</span>
                <select
                  className="wpf-subpage-form-combo"
                  {...{ [FIELD_FOCUS_KEY]: 'qcRemark' }}
                  value={h.qcRemark}
                  onChange={(e) => doc.updateHeader('qcRemark', e.target.value)}
                >
                  {QC_REMARKS.map((p) => (
                    <option key={p} value={p}>
                      {p}
          ...
          <section className="si-section si-section--grow si-section--lines-panel">
            <SalesReturnLineItemsGrid doc={doc} gridRef={gridRef} onExitGridEnd={focusNarration} />
          </section>
        ...
        <SalesReturnTotalsRail displayTotals={doc.displayTotals} />
      </FormKeyboardScope>
      </LoadingHost>
