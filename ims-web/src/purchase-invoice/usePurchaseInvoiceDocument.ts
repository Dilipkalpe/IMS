...
import { normalizePaymentType } from '../sales-invoice/invoicePayment';
import {
...
export function usePurchaseInvoiceDocument(tabId: string) {
  const ws = usePurchaseInvoiceWorkspace();
  const doc = ws.getDocument(tabId);

  const taxContext = useMemo(
    () => purchaseGstContext(doc.header),
    [doc.header.placeOfSupply, doc.header.companyGstin, doc.header.supplierGstin],
  );

  const lineDisplayMap = useMemo(
    () => buildLineDisplayMap(doc.lines, doc.header),
    [doc.lines, doc.header.placeOfSupply, doc.header.companyGstin, doc.header.supplierGstin],
  );
  ...
  const canRecordPayment = useMemo(
    () =>
      ...
      Boolean(doc.header.supplier?.trim()),
    [doc.header.paymentType, doc.header.supplier, totals.balanceDue],
  );
  ...
  const updateHeader = useCallback(
    <K extends keyof PurchaseInvoiceHeader>(key: K, value: PurchaseInvoiceHeader[K]) => {
  ...
  const updateLine = useCallback(
    (id: string, patch: Partial<PurchaseInvoiceLineItem> | PurchaseInvoiceLineItem) => {
