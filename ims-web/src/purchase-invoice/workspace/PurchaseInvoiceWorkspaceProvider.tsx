...
import { usePurchaseInvoiceNavIntent, type PurchaseInvoiceOpenIntent } from '../context/PurchaseInvoiceNavIntent';
import { invalidatePurchaseInvoiceList } from '../repository/listCache';
import { recordToEditor } from '../repository/recordMappers';
import { usePurchaseInvoiceRepository } from '../repository/PurchaseInvoiceRepositoryContext';
import type { PurchaseInvoiceHeader, PurchaseInvoiceLineItem, PurchaseInvoiceTab } from '../types';
...
interface PurchaseInvoiceWorkspaceContextValue {
  tabs: PurchaseInvoiceTab[];
  activeTabId: string;
  activeTab: PurchaseInvoiceTab | undefined;
  tabIds: string[];
  ...
  openDocumentInNewTab: (intent: PurchaseInvoiceOpenIntent) => Promise<void>;
  patchHeader: (tabId: string, key: keyof PurchaseInvoiceHeader, value: PurchaseInvoiceHeader[keyof PurchaseInvoiceHeader]) => void;
  patchLine: (tabId: string, lineId: string, patch: Partial<PurchaseInvoiceLineItem> | PurchaseInvoiceLineItem) => void;
...
const PurchaseInvoiceWorkspaceContext = createContext<PurchaseInvoiceWorkspaceContextValue | null>(null);

export function PurchaseInvoiceWorkspaceProvider({
  children,
  lineCount = 0,
}: {
  children: ReactNode;
  ...
  const repository = usePurchaseInvoiceRepository();
  const { consumeOpenIntent } = usePurchaseInvoiceNavIntent();
  const initialTabId = 'tab-1';
  const [tabs, setTabs] = useState<PurchaseInvoiceTab[]>(() => [
  ...
  const loadIntoTab = useCallback(
    async (tabId: string, intent: PurchaseInvoiceOpenIntent) => {
      ...
      try {
        ...
        if (intent.type === 'edit') {
          ...
          const prefix = normalizeDocPrefix(fresh.header.entryDocPrefix, 'PI');
  ...
  const openDocumentInNewTab = useCallback(
    async (intent: PurchaseInvoiceOpenIntent) => {
  ...
  const patchHeader = useCallback(
    (tabId: string, key: keyof PurchaseInvoiceHeader, value: PurchaseInvoiceHeader[keyof PurchaseInvoiceHeader]) => {
      updateDocument(tabId, (d) => {
  ...
  const patchLine = useCallback(
    (tabId: string, lineId: string, patch: Partial<PurchaseInvoiceLineItem> | PurchaseInvoiceLineItem) => {
      updateDocument(tabId, (d) => {
        const lines = d.lines.map((row) => {
          if (row.id !== lineId) return row;
          return typeof patch === 'object' && 'id' in patch && patch.id === lineId
            ? (patch as PurchaseInvoiceLineItem)
            : { ...row, ...patch };
        });
  ...
  const commitPrefix = useCallback(
    async (tabId: string) => {
      ...
      const prefix = normalizeDocPrefix(doc.header.entryDocPrefix, 'PI');
      const next = await repository.peekNextNo(prefix);
      updateDocument(tabId, (d) => {
        const header = {
          ...d.header,
  ...
  const duplicateToNewTab = useCallback(
    async (sourceTabId: string) => {
      ...
      const id = `tab-${tabCounter}`;
      setTabCounter((c) => c + 1);
      const prefix = normalizeDocPrefix(source.header.entryDocPrefix, 'PI');
      const next = await repository.peekNextNo(prefix);
      const header = {
        ...source.header,
      ...
      setTabs((prev) => [
        ...prev.map((t) => ({ ...t, isSelected: false })),
        newTabUi(id, 'Duplicated bill', true),
      ]);
      setDocuments((prev) => ({
        ...prev,
  ...
  const value = useMemo(
    (): PurchaseInvoiceWorkspaceContextValue => ({
      tabs,
      activeTabId,
      activeTab,
      tabIds,
  ...
  return (
    <PurchaseInvoiceWorkspaceContext.Provider value={value}>{children}</PurchaseInvoiceWorkspaceContext.Provider>
  );
}

export function usePurchaseInvoiceWorkspace(): PurchaseInvoiceWorkspaceContextValue {
  const ctx = useContext(PurchaseInvoiceWorkspaceContext);
  if (!ctx) throw new Error('PurchaseInvoiceWorkspaceProvider is required.');
  return ctx;
}
