...
export function markDocumentDirty(state: TabDocumentState, header: PurchaseInvoiceHeader, lines: PurchaseInvoiceLineItem[]): TabDocumentState {
...
export function tabTitleFromState(state: TabDocumentState): string {
  const prefix = state.header.entryDocPrefix || 'PI';
  const no = state.header.billNo?.trim();
  if (no) return `${prefix}-${no}${state.isDirty ? ' *' : ''}`;
  return `New Invoice${state.isDirty ? ' *' : ''}`;
}

export function newTabUi(id: string, title: string, selected = false): PurchaseInvoiceTab {
