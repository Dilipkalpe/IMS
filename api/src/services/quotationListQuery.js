...
import { toQuotationListItem } from './quotationListSummary.js';

const LIST_FIELDS =
  'qtPrefix docNo formattedDocNo quoteDate billDate customer status totals lines';
...
export function resolveQuotationSort(sort, sortDir) {
...
export async function fetchQuotationListPage(filter, sort, sortDir, skip, limit) {
  const { spec, useAggregation } = resolveQuotationSort(sort, sortDir);
...
export async function fetchQuotationListResponse(filter, sort, sortDir, skip, limit) {
  const [docs, total] = await Promise.all([
    fetchQuotationListPage(filter, sort, sortDir, skip, limit),
    Quotation.countDocuments(filter)
  ]);
  return {
    items: docs.map(toQuotationListItem),
    total
  };
}
